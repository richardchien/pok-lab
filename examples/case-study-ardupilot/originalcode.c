...
void loop() {
    ...
    if (RTL == 0) // Verify the RTl option (0=Waypoint mode, 1 = Return Home mode)
    {
        if ((wp_distance < 30)
            && (jumplock_wp
                == 0x00)) // Checking if the waypoint distance is less than 30, and check if the lock is open
        {
    ...
    } else // RTL set
    {
        if (digitalRead(4) == LOW) // Checks the MUX pin to see if we are in manual mode (Low = manual)
        {
            wp_alt[0] = alt; // updates the altitude till we switch to automode, then the autopilot will try to mantain
                             // that altitude
            reset_PIDs(); // Reset all the PIDs
            middle_measurement_lock = 0; // unlocks the starting positions of throttle and yaw. See the function in
                                         // "AutoPilot" tab, inside the throttle_control() function
        }
...
/**************************************************************
 * Special module to limit float values, which is like constrain() but for variables IEEE 754 (Don't worry about it,
 *it's just a floating point variables). =) It's called "ACME" because that's the generic name for everything in the
...
void throttle_control(void) {
    if ((middle_measurement_lock == 0)
        && (digitalRead(4) == HIGH)) // Verify if the lock is open (equal to zero) and if we are in automode..
    {
        ...
        middle_thr = ((read_servo - min16_throttle) * 180L)
    ...
    // Example: If you launch you airp. at 300 meters above sea level, and you want to maintain 50 meters above your
    // head, the airp. will fly at 350 meters above sea level.
...
void yaw_control(void) {
    wp_bearing =
        get_gps_course(lat,
                       lon,
                       wp_lat[current_wp],
                       wp_lon[current_wp]); // Calculating Bearing, this function is located in the GPS_Navigation tab..
    wp_distance =
        get_gps_dist(lat,
                     lon,
                     wp_lat[current_wp],
                     wp_lon[current_wp]); // Calculating Distance, this function is located in the GPS_Navigation tab..

    // test=middle_yaw+PID_heading(compass_error(wp_bearing, course)); //Central Position + PID(compass_error(desired
            ...
            if (buffer[counter] == 0x0A) // Looks for \F
            {
                ...
                if (strncmp(buffer, head_rmc, 5) == 0) // looking for rmc head....
                {
                    ...
                    if (checksum_received == checksum) // Checking checksum
                    {
                        ...
                        /*THE GPRMC order is: UTC, UTC status ,Lat, N/S indicator, Lon, E/W indicator, speed, course,
                         * date, mode, checksum*/
                        ...
                        // Longitude in degrees, decimal minutes. (ej. 4750.1234 degrees decimal minutes = 47.835390
                        // decimal degrees) Where 47 are degrees and 50 the minutes and .1234 the decimals of the
                        // minutes. To convert to decimal degrees, devide the minutes by 60 (including decimals),
                        // Example: "50.1234/60=.835390", then add the degrees, ex: "47+.835390=47.835390" decimal
                        // degrees
                        ...
                        // First i convert the decimal minutes to degrees decimals stored in "temp3", example:
                        // 501234/600000= .835390 Then i add the degrees stored in "temp" and add the result from the
                        // first step, example 47+.835390=47.835390 The result is stored in "lat" variable...
                ...
                if (strncmp(buffer, head_gga, 5) == 0) // now looking for GPGGA head....
                {
                    ...
                    if (checksum_received == checksum) // Checking checksum
                    {
                        ...
                        token = strtok_r(
                            NULL, search, &brkb); // ALTITUDE, is the only meaning of this string.. in meters of course.
                        alt = atoi(token);
                        if (alt < 0) {
                            alt = 0;
                        }
...
void init_ardupilot(void) {
    ...
    // When you put the switch in the central position the attiny will set high a pin called "mode", and you can use it
    // to do whatever yowant...
...
void init_startup_parameters(void) {
    ...
    yaw_control(); // I've put this here because i need to calculate the distance to the next waypoint, otherwise it
                   // will start at waypoint 2.
    ...
    wp_alt[0] = 100; // Storing home altitude plus 100, otherwise it will try to maintain the launch altitude, you
                     // really don't want fly too low..
...
int PID_heading(int PID_error) {
    ...
    float dt =
        (float)(millis() - heading_PID_timer) / 1000; // calculating dt, you must divide it by 1000, because this system
    ...
    heading_output =
        (kp[0] * PID_error); // Proportional part, is just the KP constant * error.. and addidng to the output
...
int PID_altitude(int PID_set_Point, int PID_current_Point) {
    ...
    float dt = (float)(millis() - altitude_PID_timer)
               / 1000; // calculating dt, you must divide it by 1000, because this
                       // system only undestand seconds.. and is normally given in millis
 ...
 ***************************************************************/
void Init_servo(
    void) // This part will configure the PWM to control the servo 100% by hardware, and not waste CPU time..
{
    ...
    // Note: these strange strings that follow, like OCRI1A, are actually predefined Atmega168 registers. We load the
    // registers and the chip does the rest.

    // Remember the registers not declared here remains zero by default...
    TCCR1A =
        ((1 << WGM11) | (1 << COM1B1) | (1 << COM1A1)); // Please read page 131 of DataSheet, we are changing the
                                                        // registers settings of WGM11,COM1B1,COM1A1 to 1 thats all...
    TCCR1B = (1 << WGM13) | (1 << WGM12)
             | (1 << CS11); // Prescaler set to 8, that give us a resolution of 2us, read page 134 of data sheet
...
unsigned long PulseIn(uint8_t pin, unsigned long timeout) {
    ...
    while ((*portInputRegister(port) & bit) == bit) width++;
    interrupts();
