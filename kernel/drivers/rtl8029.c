...
#define NE2000_SELECT_PAGE(dev, page) \
    outb((dev)->addr, (inb((dev)->addr + NE2000_CR) & ~(NE2000_CR_PS0 | NE2000_CR_PS1)) | ((page) << 6))

static size_t ne2000_write(const s_ne2000_dev* dev, const void* buf, unsigned short count, unsigned short offset) {
    const char* p = NULL;
    size_t ret = count;

    // Sets RD2 (abort/complete remote DMA)
    outb((dev)->addr, (inb((dev)->addr + NE2000_CR) & ~(NE2000_CR_RD0 | NE2000_CR_RD1)) | NE2000_CR_RD2);
    ...
    outb((dev)->addr, (inb((dev)->addr + NE2000_CR) & ~(NE2000_CR_RD0 | NE2000_CR_RD2)) | NE2000_CR_RD1);
...
static size_t ne2000_read(const s_ne2000_dev* dev, void* buf, unsigned short count, unsigned short offset) {
    char* p = NULL;
    size_t ret = count;
    ...
    outb((dev)->addr, (inb((dev)->addr + NE2000_CR) & ~(NE2000_CR_RD1 | NE2000_CR_RD2)) | NE2000_CR_RD0);
...
void rtl8029_poll_and_read(pok_port_id_t port_id, void* data, uint32_t len) {
    ...
    if (state && (state & NE2000_ISR_PRX)) {
        ...
        while (1) {
            ...
            ne2000_read(&dev, &recv_packet, ne2000_hdr.size - sizeof(s_ne2000_header), offset);
            ...
            if (pok_global_ports_to_local_ports[recv_packet.udp.dst] != invalid_local_port) {
                ...
                printf("\tsrc global port %d with an invalid port id for the destination (%d)\n",
                       ...
                       recv_packet.udp.dst);
#endif
                outb(dev.addr + NE2000_BNRY, ne2000_hdr.next > NE2000_MEMSZ ? NE2000_RXBUF - 1 : ne2000_hdr.next - 1);
                break;
            }
    ...
    if (dev.msg_nb == 0) outb(dev.addr + NE2000_ISR, NE2000_ISR_PRX); // Clear PRX flag
}
/**
...
void rtl8029_poll() {
    ...
    // do we have an interrupt flag set?
    if ((state = inb(dev.addr + NE2000_ISR)) == 0) return;
...
void rtl8029_init() {
    ...
    for (i = 0; i < 6; i++) dev.mac[i] = buf[i * 2];
