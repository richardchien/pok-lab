...
typedef struct {
    ...
    uint32_t (*sched_func)(uint32_t low, uint32_t high, uint32_t prev_thread,
                           uint32_t cur_thread); /**< Scheduling function to scheduler threads */

    uint64_t activation; /**< Last activation time of the partition */
    uint32_t prev_thread; /**< member for the scheduler (previous scheduled real thread inside the partition,i.e not the
                             idle thread */
 ...
 */
static inline bool_t pok_check_ptr_in_partition(pok_partition_id_t pid, void *ptr, uint32_t size) {
    if (pid >= POK_CONFIG_NB_PARTITIONS) return FALSE;

    uint32_t psize = pok_partitions[pid].size;

    if (size > psize) return FALSE;

    if ((uint32_t)ptr <= (psize - size)) return TRUE;
