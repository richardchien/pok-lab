...
#if defined(POK_NEEDS_CONSOLE) || defined(POK_NEEDS_DEBUG) || defined(POK_NEEDS_INSTRUMENTATION) \
    || defined(POK_NEEDS_COVERAGE_INFOS) || defined(POK_NEEDS_USER_DEBUG)

static void write_serial(char a) {
    while ((inw(UART1 + UART_STAT_OFFSET) & UART_STATUS_THE) == 0) continue;

    outb(UART1 + UART_DATA_OFFSET, (uint8_t)a);
}

pok_bool_t pok_cons_write(const char *s, size_t length) {
    for (; length > 0; length--) write_serial(*s++);
    return 0;
}
