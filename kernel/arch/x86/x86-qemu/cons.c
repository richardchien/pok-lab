...
#if defined(POK_NEEDS_DEBUG) || defined(POK_NEEDS_CONSOLE) || defined(POK_NEEDS_INSTRUMENTATION) \
...
void read_serial(char *a) {
    if (is_received_data_ready() != 0) *a = inb(COM1);
}
#endif
...
#if defined(POK_NEEDS_CONSOLE) || (POK_NEEDS_DEBUG) || defined(POK_NEEDS_INSTRUMENTATION) \
