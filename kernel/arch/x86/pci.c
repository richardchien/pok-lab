...
static inline unsigned int pci_read(unsigned int bus, unsigned int dev, unsigned int fun, unsigned int reg) {
        ...
        for (dev = 0; dev < PCI_DEV_MAX; dev++)
            for (fun = 0; fun < PCI_FUN_MAX; fun++)
                if (((unsigned short)pci_read(bus, dev, fun, PCI_REG_VENDORID)) == d->vendorid
                    ...
                    if (pci_read(bus, dev, fun, PCI_REG_HEADERTYPE) != 0) continue;
...
pok_ret_t pci_register(s_pci_device* dev, uint8_t part_id) {
    if (pci_open(dev) != 0) return (-1);

    pok_partitions[part_id].io_min = (dev->bar[0] & (~0x1F));
    pok_partitions[part_id].io_max = (dev->bar[0] & (~0x1F)) + dev->io_range;

    pok_idt_set_gate(32 + dev->irq_line, GDT_CORE_CODE_SEGMENT, (uint32_t)pci_handler, IDTE_INTERRUPT, 0);
