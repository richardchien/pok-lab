...
pok_ret_t pok_create_space(uint8_t partition_id, uint32_t addr, uint32_t size) {
    gdt_set_segment(GDT_PARTITION_CODE_SEGMENT(partition_id), addr, size, GDTE_CODE, 3);

    gdt_set_segment(GDT_PARTITION_DATA_SEGMENT(partition_id), addr, size, GDTE_DATA, 3);
...
uint32_t pok_space_context_create(uint8_t partition_id, uint32_t entry_rel, uint32_t stack_rel, uint32_t arg1,
                                  uint32_t arg2) {
    char* stack_addr;
    space_context_t* sp;
    ...
    sp = (space_context_t*)(stack_addr + KERNEL_STACK_SIZE - 4 - sizeof(space_context_t));
...
void pok_dispatch_space(uint8_t partition_id, uint32_t user_pc, uint32_t user_sp, uint32_t kernel_sp, uint32_t arg1,
                        uint32_t arg2) {
    interrupt_frame ctx;
    uint32_t code_sel;
    uint32_t data_sel;
