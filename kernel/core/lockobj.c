...
pok_ret_t pok_lockobj_create(pok_lockobj_t* obj, const pok_lockobj_attr_t* attr) {
    ...
    if ((attr->locking_policy != POK_LOCKOBJ_POLICY_STANDARD) && (attr->locking_policy != POK_LOCKOBJ_POLICY_PIP)
     ...
     */
    if ((attr->kind != POK_LOCKOBJ_KIND_MUTEX) && (attr->kind != POK_LOCKOBJ_KIND_SEMAPHORE)
...
pok_ret_t pok_lockobj_eventsignal(pok_lockobj_t* obj) {
    ...
    for (tmp = 0; tmp < POK_CONFIG_NB_THREADS; tmp++) {
        if (tmp == POK_SCHED_CURRENT_THREAD) continue;
