...
pok_ret_t pok_port_queueing_receive(const pok_port_id_t id, const uint64_t timeout, const pok_port_size_t maxlen,
                                    void* data, pok_port_size_t* len);

pok_ret_t pok_port_queueing_send(const pok_port_id_t id, const void* data, const pok_port_size_t len,
...
#define pok_port_queueing_id(name, id) pok_syscall2(POK_SYSCALL_MIDDLEWARE_QUEUEING_ID, (uint32_t)name, (uint32_t)id)
/*
 * Similar to:
...
#define pok_port_sampling_id(name, id) pok_syscall2(POK_SYSCALL_MIDDLEWARE_SAMPLING_ID, (uint32_t)name, (uint32_t)id)
/*
 *    Similar to
