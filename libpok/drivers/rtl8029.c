...
void rtl8029_poll_and_read(pok_port_id_t port_id, void* data, uint32_t len) {
    pok_syscall3(POK_SYSCALL_RTL8929_POLL_AND_READ, (uint32_t)port_id, (uint32_t)data, len);
}
/**
...
void rtl8029_read(pok_port_id_t port_id, void* data, uint32_t len) {
    pok_syscall3(POK_SYSCALL_RTL8929_READ, (uint32_t)port_id, (uint32_t)data, len);
}
...
void rtl8029_write(pok_port_id_t port_id, const void* data, uint32_t len) {
    pok_syscall3(POK_SYSCALL_RTL8929_WRITE, (uint32_t)port_id, (uint32_t)data, len);
}
