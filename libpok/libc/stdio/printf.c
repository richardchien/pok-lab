...
static void my_putc(char c, struct s_file* file) {
    ...
    if (file->pos == MY_BUF_SIZE) my_fflush(file);
}
...
static int my_printnbr_base(uint32_t n, const char base[], int card, struct s_file* file) {
    int digits[96];
    int i;
    int count;
